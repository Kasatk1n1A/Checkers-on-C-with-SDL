...
```
## SDL_DestroyTexture()
**SDL_DestroyTexture()** — это функция в библиотеке SDL (Simple DirectMedia Layer), которая используется для освобождения ресурсов, связанных с текстурой SDL_Texture. Она необходима для корректного удаления текстур и предотвращения утечек памяти в приложении.
### Синтаксис[![](https://wiki.libsdl.org/static_files/link.svg)](https://wiki.libsdl.org/SDL2/SDL_DestroyTexture#syntax)

```
void SDL_DestroyTexture(SDL_Texture * texture);
```
Функция SDL_DestroyTexture принимает один параметр:

- texture — текстура (SDL_Texture), которую нужно удалить.

После вызова функции SDL_DestroyTexture текстура больше не может быть использована, и все ссылки на неё становятся недействительными.

Важно вызывать SDL_DestroyTexture для каждой созданной текстуры, когда она больше не нужна, чтобы освободить занятые ресурсы и обеспечить корректную работу приложения.
## SDL_DestroyWindow()
...
## SDL_RenderCopy()
**SDL_RenderCopy()** — это функция в библиотеке SDL (Simple DirectMedia Layer), которая используется для копирования текстуры на экран с помощью указанного рендера. Она позволяет отображать загруженные изображения, спрайты и другие графические элементы в окне приложения.
### Синтаксис[![](https://wiki.libsdl.org/static_files/link.svg)](https://wiki.libsdl.org/SDL2/SDL_RenderCopy#syntax)
...
```
Функция SDL_RenderCopy принимает четыре параметра:

1. **renderer** — объект рендера (SDL_Renderer), который будет использоваться для отображения текстуры.
2. **texture** — текстура (SDL_Texture), которую нужно отобразить.
3. **srcrect** — указатель на структуру SDL_Rect, определяющую прямоугольник источника на текстуре, который будет скопирован. Если этот параметр равен NULL, будет скопирована вся текстура.
4. **dstrect** — указатель на структуру SDL_Rect, определяющую прямоугольник назначения на экране, куда будет скопирована текстура. Если этот параметр равен NULL, текстура будет скопирована в исходном размере и положении.

При успешном выполнении функция SDL_RenderCopy возвращает 0. В случае ошибки возвращается -1, и можно использовать функцию SDL_GetError для получения дополнительной информации об ошибке.

Использование SDL_RenderCopy позволяет разработчикам легко отображать текстуры на экране, контролировать их положение, размер и другие параметры, что делает её важным инструментом для создания графики в приложениях SDL.

### Замечания[![](https://wiki.libsdl.org/static_files/link.svg)](https://wiki.libsdl.org/SDL2/SDL_RenderCopy#remarks)

Текстура смешивается с целевым изображением в соответствии с режимом наложения, заданным с помощью [SDL_SetTextureBlendMode](https://wiki.libsdl.org/SDL2/SDL_SetTextureBlendMode)().

Цвет текстуры зависит от цветовой модуляции, заданной [SDL_SetTextureColorMod](https://wiki.libsdl.org/SDL2/SDL_SetTextureColorMod)().

Альфа-канал текстуры изменяется в зависимости от его альфа-модуляции, заданной [SDL_SetTextureAlphaMod](https://wiki.libsdl.org/SDL2/SDL_SetTextureAlphaMod)().
## SDL_RenderClear()
...
```
## SDL_Rect()
**SDL_Rect()** — это структура в библиотеке SDL (Simple DirectMedia Layer), которая представляет прямоугольник. Она используется для определения положения и размера различных элементов, таких как текстуры, окна и другие графические объекты.

### Синтаксис[![](https://wiki.libsdl.org/static_files/link.svg)](https://wiki.libsdl.org/SDL2/SDL_Rect#syntax)
...
```
Структура SDL_Rect включает следующие поля:

- **x** — координата по оси X верхнего левого угла прямоугольника.
- **y** — координата по оси Y верхнего левого угла прямоугольника.
- **w** — ширина прямоугольника.
- **h** — высота прямоугольника.

SDL_Rect используется в различных функциях SDL, таких как SDL_RenderCopy, для определения положения и размера текстур на экране, а также в других контекстах, где необходимо представить прямоугольную область. Это делает её важным инструментом для работы с графикой и позиционированием объектов в приложениях SDL.
...
## SDL_Texture
**SDL_Texture** — это структура в библиотеке SDL (Simple DirectMedia Layer), которая представляет текстуру, то есть изображение, готовое к отображению на экране. Текстуры используются для рендеринга графики в приложениях, работающих с SDL.

Текстуры создаются из различных источников, таких как файлы изображений (например, PNG, JPEG), поверхности SDL_Surface или напрямую из памяти. Они оптимизированы для быстрого отображения на экране и могут быть отмасштабированы, повернуты и преобразованы без потери качества.

Для работы с текстурами в SDL необходимо выполнить следующие шаги:

1. Создать объект рендера (SDL_Renderer) с помощью функции SDL_CreateRenderer.
2. Загрузить изображение или создать поверхность (SDL_Surface).
3. Создать текстуру с помощью функции SDL_CreateTextureFromSurface или SDL_CreateTexture.
4. Настроить параметры текстуры, такие как формат пикселей, размер и фильтры масштабирования.
5. Отобразить текстуру на экране с помощью функции SDL_RenderCopy или SDL_RenderCopyEx.
6. Освободить ресурсы, связанные с текстурой, когда они больше не нужны, с помощью функции SDL_DestroyTexture.

SDL_Texture предоставляет множество возможностей для работы с графикой, включая поддержку прозрачности, наложения текстур и применения различных эффектов. Это делает его мощным инструментом для разработки игр и других графических приложений.
## SDL_Window
